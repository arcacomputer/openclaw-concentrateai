// Exact-package gateway smoke. Raw RPC envelope retained separately from normalized host result.
...
import {evaluateFeature} from '/tmp/candidate/scripts/verification/feature-acceptance.mjs';
const acceptanceSha256=createHash('sha256').update(readFileSync('/tmp/candidate/scripts/verification/feature-acceptance.mjs')).digest('hex');
const row=JSON.parse(process.env.BATCH_ROW);delete process.env.BATCH_ROW;assert.equal(row.feature,'basic-response','Gateway smoke is a separate text-only qualification');
...
save('metadata.json',{at:new Date().toISOString(),protocol:'precision-v2',acceptanceSha256,imageFixtureSha256:createHash('sha256').update(readFileSync('/tmp/image.png')).digest('hex'),node:process.version,openclaw:'2026.9.4',case:row,packageLockSha256:createHash('sha256').update(readFileSync('/tmp/host/package-lock.json')).digest('hex')});
...
async function run(label,recall=false){const home='/tmp/feature-'+row.index+'-'+mode;activeWorkspace=home+'/workspace';mkdirSync(activeWorkspace,{recursive:true});const toolValue='T-'+randomUUID();writeFileSync(activeWorkspace+'/feature.txt',toolValue+'\n');save(mode+'-tool-value.json',{value:toolValue});if(row.feature==='parallel'&&!recall){const values={a:'A-'+randomUUID(),b:'B-'+randomUUID()};writeFileSync(activeWorkspace+'/a.txt',values.a);writeFileSync(activeWorkspace+'/b.txt',values.b);save('fixture-values.json',values);}writeFileSync(activeWorkspace+'/image.png',readFileSync('/tmp/image.png'));
 ...
 const env={PATH:process.env.PATH,HOME:home,OPENCLAW_CONFIG_PATH:home+'/config.json',OPENCLAW_STATE_DIR:home,CONCENTRATE_API_KEY:'synthetic-local-proxy',NODE_OPTIONS:'--import=/tmp/credential-audit.mjs',AUDIT_PROOF:proof+'/'+label+'-child-isolation.json'};
 const stop=new AbortController();
 const gateway=runProcessGroup('/tmp/host/node_modules/.bin/openclaw',['gateway','run','--port','18799','--bind','loopback'],{env:{...env,AUDIT_PROOF:proof+'/'+label+'-gateway-isolation.json'},timeoutMs:80000,signal:stop.signal});
 try {
  let ready=false;const deadline=Date.now()+18000;
  while(Date.now()<deadline){try{const response=await fetch('http://127.0.0.1:18799/readyz',{signal:AbortSignal.timeout(750)});await response.text();if(response.ok){ready=true;break;}}catch{}await new Promise(r=>setTimeout(r,150));}
  assert.equal(ready,true,'Gateway readiness deadline');
  const r=await runProcessGroup('/tmp/host/node_modules/.bin/openclaw',args.filter(arg=>arg!=='--local'),{env,timeoutMs:60000});
  save(label+'-gateway-client.json',r);
  const envelope=JSON.parse(r.stdout);
  assert.equal(envelope.status,'ok','Gateway RPC result, not silent local fallback');
  assert.equal(typeof envelope.runId,'string');assert.ok(envelope.result?.meta?.agentMeta);
  save(label+'-gateway-route.json',{runIdPresent:true,status:envelope.status,localFallback:false,rawStdoutSha256:createHash('sha256').update(r.stdout).digest('hex')});
  const normalized={...r,stdout:JSON.stringify(envelope.result)};
  save(label+'-host.json',normalized);return normalized;
 } finally {stop.abort();save(label+'-gateway-stop.json',await gateway);}

}
try{
 let s;if(row.syntheticOnly||row.preflightOnly){assert.equal(secret,undefined);save('parent-isolation.json',{at:new Date().toISOString(),paidCredentialPresent:false,environmentKeys:Object.keys(process.env).sort()});s=await run('synthetic');await inFlight;if(row.feature==='parallel'){const recall=await run('synthetic-recall',true);await inFlight;assert.equal(recall.status,0);}}else{const prior='/tmp/proof/'+row.preflightIndex;const gate=JSON.parse(readFileSync(prior+'/preflight-result.json'));assert.equal(gate.passed,true);assert.equal(gate.id,row.id);assert.equal(gate.feature,row.feature);assert.equal(gate.acceptanceSha256,acceptanceSha256);assert.equal(gate.sourceSha256,createHash('sha256').update(readFileSync('/tmp/batch.mjs')).digest('hex'));s=JSON.parse(readFileSync(prior+'/synthetic-host.json'));requests.push(...JSON.parse(readFileSync(prior+'/requests.json')));syntheticCount=gate.syntheticCount;save('preflight-reused.json',gate);}
...
assert.equal(host.meta?.agentMeta?.provider,'concentrate');assert.equal(host.meta?.agentMeta?.model,row.id);
const expected=row.feature==='tool-roundtrip'?JSON.parse(readFileSync(proof+'/live-tool-value.json')).value:undefined;
const observations={id:row.id,feature:row.feature,protocol:'precision-v2',output,expected,forwarded,receiptCount:receipts.length,at:new Date().toISOString(),...evaluateFeature(row.feature,output,expected)};
observations.strictSchemaOnWire=!!row.schema;
observations.reasoningEffortOnWire=row.reasoning?'low':null;
observations.reasoningTokens=receipts.reduce((n,x)=>n+(x.usage.output_tokens_details?.reasoning_tokens??0),0);
